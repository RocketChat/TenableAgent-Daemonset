...
RUN curl -s -k $NESSUS_URL -o NessusAgent-install.deb
...
RUN echo $NESSUS_SHA256 > sha256_signature.txt
RUN sha256sum -c sha256_signature.txt

#Install Nessus

RUN dpkg -i NessusAgent-install.deb && rm -rf NessusAgent-install.deb
